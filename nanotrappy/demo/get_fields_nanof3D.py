...
y = np.linspace(0, 800e-9, 501)
x = np.linspace(0, 800e-9, 501)
z = np.array([0])
...
for k in range(len(x)):
    for i in range(len(y)):
        for j in range(len(z)):
            E_blue[:, k, i, j] = struct_nf.electric_field_linear(x[k], y[i], z[j], wavelength_blue, 1, 0)
...
np.save("./testfolder/modeblue685.npy", [wavelength_blue, x, y, z, E_blue])
