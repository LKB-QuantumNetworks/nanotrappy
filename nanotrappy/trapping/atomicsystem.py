...
class atomicsystem():
    ...
    def __init__(self,atom,state,f):
        self._atom = atom
        self.rangeN = 16
        self.Nground = self._atom.groundStateN
        self.state = check_and_parse(state)
        self._f = f
        self.f0, self.fp, self.fm, self.f0fm, self.f0fp, self.fmfp, self.f0f0, self.fmfm, self.fpfp = F0(f), Fp(f), Fm(f), F0Fm(f), F0Fp(f), FmFp(f), F0F0(f), FmFm(f), FpFp(f)
        self.id = np.eye(2*self.f+1,2*self.f+1)

        pol = arc.calculations_atom_single.DynamicPolarizability(self._atom, self.state.n, self.state.l, self.state.j)
    ...
    def f(self, value):
        self._f = value
        self.f0, self.fp, self.fm, self.f0fm, self.f0fp, self.fmfp, self.f0f0, self.fmfm, self.fpfp = F0(value), Fp(value), Fm(value), F0Fm(value), F0Fp(value), FmFp(value), F0F0(value), FmFm(value), FpFp(value)
        self.id = np.eye(2*self.f+1,2*self.f+1)
        ...
        if self._state.j == 1/2 and self._state.l == 0 and self.Nground == self._state.n:
            ...
            for jnumber in [3/2,5/2]:
                for nnumber in range(self.Nground-1,self.Nground+2+self.rangeN+1):
                    ...
                    self.dicoatom[(nnumber,D,jnumber)]=(self.atom.getTransitionFrequency(self.excitedstate.n,self.excitedstate.l,self.excitedstate.j,nnumber,D,jnumber),self.atom.getReducedMatrixElementJ(self.excitedstate.n,self.excitedstate.l,self.excitedstate.j,nnumber,D,jnumber),self.atom.getTransitionRate(nnumber,D,jnumber,self.excitedstate.n,self.excitedstate.l,self.excitedstate.j))
        
        if self._state == self.groundstate :
            wigner6j = np.zeros((3,np.shape(np.arange(abs(self._state.j-self.atom.I),abs(self._state.j+self.atom.I)+1,1))[0],np.shape(np.arange(abs(5/2-self.atom.I),abs(5/2+self.atom.I)+1,1))[0]))
            for f in np.arange(abs(self._state.j-self.atom.I),abs(self._state.j+self.atom.I)+1,1):
                for fsum in np.arange(abs(3/2-self.atom.I),abs(3/2+self.atom.I)+1,1):
                    wigner6j[0,int(f-abs(self._state.j-self.atom.I)),int(fsum-1)] = wigner_6j(self._state.j,1/2,1,fsum,f,self.atom.I)
                    wigner6j[1,int(f-abs(self._state.j-self.atom.I)),int(fsum-1)] = wigner_6j(self._state.j,3/2,1,fsum,f,self.atom.I)
        elif self._state == self.excitedstate:
            wigner6j = np.zeros((3,np.shape(np.arange(abs(self._state.j-self.atom.I),abs(self._state.j+self.atom.I)+1,1))[0],np.shape(np.arange(abs(5/2-self.atom.I),abs(5/2+self.atom.I)+1,1))[0]))
            for f in np.arange(abs(self._state.j-self.atom.I),abs(self._state.j+self.atom.I)+1,1):
                for fsum in np.arange(abs(5/2-self.atom.I),abs(5/2+self.atom.I)+1,1):
                    wigner6j[0,int(f-abs(self._state.j-self.atom.I)),int(fsum-1)] = wigner_6j(self._state.j,1/2,1,fsum,f,self.atom.I)
                    wigner6j[1,int(f-abs(self._state.j-self.atom.I)),int(fsum-1)] = wigner_6j(self._state.j,3/2,1,fsum,f,self.atom.I)
                    wigner6j[2,int(f-abs(self._state.j-self.atom.I)),int(fsum-1)] = wigner_6j(self._state.j,5/2,1,fsum,f,self.atom.I)
    ...
    def alpha_scalar(self,lmbda):
        ...
        """
        if self.f > self.atom.I + self.state.j or self.f < abs(self.atom.I - self.state.j):
        ...
        for level in coupledlevels:
            ...
            for fsum in np.arange(abs(level.j-self.atom.I),abs(level.j+self.atom.I)+1,1):
                tot += rde*np.conj(rde)*freq*(2*fsum +1)*(self.wigner6j[int(level.j-1/2),int(self.f-abs(self.state.j-self.atom.I)),int(fsum-1)])**2
    ...
    def alpha_vector(self,lmbda):
        ...
        for level in coupledlevels:
            ...
            freq = np.real(1/(sign*omega-(2*np.pi*cc/lmbda)-1j*gamma/2)-1/(sign*omega+(2*np.pi*cc/lmbda)+1j*gamma/2))
            for fsum in np.arange(abs(level.j-self.atom.I),abs(level.j+self.atom.I)+1,1):
                tot += rde*np.conj(rde)*(-1)**(self.f+fsum)*np.sqrt(6*self.f*(2*self.f+1)/(self.f+1))*wigner6j12(1,1,1,self.f,self.f,fsum)*freq*(2*fsum +1)*self.wigner6j[int(level.j-1/2),int(self.f-abs(self.state.j-self.atom.I)),int(fsum-1)]**2
    ...
    def alpha_tensor(self,lmbda):
        ...
        for level in coupledlevels:
            ...
            freq = np.real(1/(sign*omega-(2*np.pi*cc/lmbda)-1j*gamma/2)+1/(sign*omega+(2*np.pi*cc/lmbda)+1j*gamma/2))
            for fsum in np.arange(abs(level.j-self.atom.I),abs(level.j+self.atom.I)+1,1):
                tot += rde*np.conj(rde)*(-1)**(self.f+fsum)*np.sqrt(10*self.f*(2*self.f+1)*(2*self.f-1)/(3*(self.f+1)*(2*self.f+3)))*wigner6j12(1,1,2,self.f,self.f,fsum)*freq*(2*fsum +1)*self.wigner6j[int(level.j-1/2),int(self.f-abs(self.state.j-self.atom.I)),int(fsum-1)]**2
        tot = (1/(hbar))*au**2*tot
        return tot

    def deltascalar(self,Ep,Em,E0):
        return self.id*((Em*np.conj(Em) + Ep*np.conj(Ep) + E0*np.conj(E0))/4)

    def deltavector(self,Ep,Em,E0):
        return (1/(2*self.f))*(2*(Ep*np.conj(Ep)/4-Em*np.conj(Em)/4)*self.f0 + np.sqrt(2)*((E0*np.conj(Em)/4+Ep*np.conj(E0)/4)*self.fm + (E0*np.conj(Ep)/4+Em*np.conj(E0)/4)*self.fp))

    def deltatensor(self,Ep,Em,E0):
        return (3*(E0*np.conj(Em)-Ep*np.conj(E0))/(4*np.sqrt(2)*self.f*(2*self.f-1)))*self.f0fm+(3*(Em*np.conj(E0)-E0*np.conj(Ep))/(4*np.sqrt(2)*self.f*(2*self.f-1)))*self.f0fp+((Em*np.conj(Em)+Ep*np.conj(Ep)-2*E0*np.conj(E0))/(8*self.f*(2*self.f-1)))*self.fmfp+((Em*np.conj(Em)+Ep*np.conj(Ep)-2*E0*np.conj(E0))/(8*self.f*(2*self.f-1)))*f0+((-Em*np.conj(Em)-Ep*np.conj(Ep)+2*E0*np.conj(E0))/(4*self.f*(2*self.f-1)))*self.f0f0+(3*(E0*np.conj(Em)-Ep*np.conj(E0))/(8*np.sqrt(2)*self.f*(2*self.f-1)))*self.fm+(3*(E0*np.conj(Ep)-Em*np.conj(E0))/(8*np.sqrt(2)*self.f*(2*self.f-1)))*self.fp-(3*Ep*np.conj(Em)/(8*self.f*(2*self.f-1)))*self.fmfm-(3*Em*np.conj(Ep)/(8*self.f*(2*self.f-1)))*self.fpfp


    def scalarshift(self,Ep,Em,E0):
        # return -1.0*self.alpha0*deltascalar(Ep,Em,E0,self.f)
        return -1.0*self.alpha0*self.deltascalar(Ep,Em,E0)

    def vectorshift(self,Ep,Em,E0):
        # return -1.0*self.alpha1*deltavector(Ep,Em,E0,self.f)
        return -1.0*self.alpha1*self.deltavector(Ep,Em,E0)

    def tensorshift(self,Ep,Em,E0):
        # return -1.0*self.alpha2*deltatensor(Ep,Em,E0,self.f)
        return -1.0*self.alpha2*self.deltatensor(Ep,Em,E0)
    ...
    def totalshift(self,Ep,Em,E0):
        ...
        return -1.0*self.alpha0*self.deltascalar(Ep,Em,E0)-1.0*self.alpha1*self.deltavector(Ep,Em,E0)-1.0*self.alpha2*self.deltatensor(Ep,Em,E0)
    ...
    def potential_partial(self,Ep,Em,E0,contrib = "scalar",show=False):
        ...
        if contrib == "scalar":
            ...
            Htemp = self.vectorshift(Ep,Em,E0)/(mK*kB)
        elif contrib =="tensor":
            Htemp = self.tensorshift(Ep,Em,E0)/(mK*kB)
        else:
    ...
    def alphaim0(self, state, omega):
        ...
        pol.defineBasis(self.groundstate.n, self.groundstate.n + self.rangeN)
