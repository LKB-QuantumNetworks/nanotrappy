...
from NanoTrap.utils.profiler import profile
...
class atomicsystem_dico():
    ...
    """
    def __init__(self,atom,state,isGround = True):
        self.atom = atom
    ...
    #        self.dicoatom = {self.groundstate:{},self.excitedstate:{}}
        rangeN = 34
        if isGround == True and self.Nground == self.state.n:
            self.groundstate = self.state
        ...
        elif isGround == False or self.Nground != self.state.n:
            ## ground state
            self.groundstate = atomiclevel(self.Nground,0,1/2)
            self.excitedstate = self.state
    ...
    def wigner6j(self):
        if self.state == self.groundstate :
            wigner6j = np.zeros((3,np.shape(np.arange(abs(self.state.j-self.atom.I),abs(self.state.j+self.atom.I)+1,1))[0],np.shape(np.arange(abs(5/2-self.atom.I),abs(5/2+self.atom.I)+1,1))[0]))
            for f in np.arange(abs(self.state.j-self.atom.I),abs(self.state.j+self.atom.I)+1,1):
                for fsum in np.arange(abs(3/2-self.atom.I),abs(3/2+self.atom.I)+1,1):
                    wigner6j[0,int(f-abs(self.state.j-self.atom.I)),int(fsum-1)] = wigner_6j(self.state.j,1/2,1,fsum,f,self.atom.I)
                    wigner6j[1,int(f-abs(self.state.j-self.atom.I)),int(fsum-1)] = wigner_6j(self.state.j,3/2,1,fsum,f,self.atom.I)
        elif self.state == self.excitedstate:
            wigner6j = np.zeros((3,np.shape(np.arange(abs(self.state.j-self.atom.I),abs(self.state.j+self.atom.I)+1,1))[0],np.shape(np.arange(abs(5/2-self.atom.I),abs(5/2+self.atom.I)+1,1))[0]))
            for f in np.arange(abs(self.state.j-self.atom.I),abs(self.state.j+self.atom.I)+1,1):
                for fsum in np.arange(abs(5/2-self.atom.I),abs(5/2+self.atom.I)+1,1):
                    wigner6j[0,int(f-abs(self.state.j-self.atom.I)),int(fsum-1)] = wigner_6j(self.state.j,1/2,1,fsum,f,self.atom.I)
                    wigner6j[1,int(f-abs(self.state.j-self.atom.I)),int(fsum-1)] = wigner_6j(self.state.j,3/2,1,fsum,f,self.atom.I)
                    wigner6j[2,int(f-abs(self.state.j-self.atom.I)),int(fsum-1)] = wigner_6j(self.state.j,5/2,1,fsum,f,self.atom.I)
        return wigner6j


    def alpha0(self,f,lmbda):
        ...
        """
        if f > self.atom.I + self.state.j or f < abs(self.atom.I - self.state.j):
...
#            rde = abs(self.atom.getReducedMatrixElementJ(state.n,state.l,state.j,level.n,level.l,level.j))
            ...
            for fsum in np.arange(abs(level.j-self.atom.I),abs(level.j+self.atom.I)+1,1):
                tot += rde*np.conj(rde)*freq*(2*fsum +1)*(self.wigner6j[int(level.j-1/2),int(f-abs(self.state.j-self.atom.I)),int(fsum-1)])**2
