...
class Outside_Simulation():
    def __init__(self,system,trap,E,x,y,z,material):
        """
        ...
        self.normalized = 0
        self.wavelengths_dict = list(self.trap.wavelengths_dict.values())   
        
        if np.shape(self.E) != np.shape(self.wavelengths_dict) :
            raise ValueError("You must specify as many wavelengths as the number of fields in the list")

    def normalize_E(self,power_blues,power_reds):
        if len(power_blues) != len(self.E[0]) or len(power_reds) != len(self.E[1]):
            raise ValueError("the input powers should be as many as the number of specified E fields")
        elif self.normalized == 1:
            raise ValueError("The fields have already been normalized, don't do it again !")
        else :
            E_blue = [{} for k in range(len(self.E[0]))]
            E_red = [{} for k in range(len(self.E[1]))]
            for j in range(len(self.E[0])):
                for keys, values in self.E[0][j].items():
                    E_blue[j][keys] = values/np.sqrt(np.abs(power_blues[j]))
            for j in range(len(self.E[1])):
                for keys, values in self.E[1][j].items():
                    E_red[j][keys] = values/np.sqrt(np.abs(power_reds[j]))
            self.E = [E_blue, E_red]
            self.normalized = 1
        return self.E
    ...
    def restrictE2D(self,plane,coord_value):
        ...
        for j in range(len(self.E[0])):#number of wavelengths at each color
            for keys, values in self.E[0][j].items():
                slc = [slice(None)] * len(self.E[0][j]["x"].shape)
                slc[coord3] = slice(index_coord,index_coord+1)
                E_blue[j][keys] = np.squeeze(values[slc])
                
        for j in range(len(self.E[1])):#number of wavelengths at each color
            for keys, values in self.E[1][j].items():
                slc = [slice(None)] * len(self.E[1][j]["x"].shape)
                slc[coord3] = slice(index_coord,index_coord+1)
                E_red[j][keys] = np.squeeze(values[slc])
...
#                 res, vec = self.getE_convert_and_compute(state,f,mf_shift,plane,out_of_plane_coord,0,wavelength_index_red)
# #                self.mask_red[plane] = res
#                 self.mask_red[plane,np.round(self.wavelengths_dict[1][wavelength_index_red],11)] = res
...
#        E = self.E[wavelength_index]
    ...
    def simulate1D(self,state,f,mf,plane,axe,coord1,coord2,wavelength_index_blue,wavelength_index_red):
        ...
        if plane == "XY":
            ...
            self.coord1 = self.x
            self.coord2 = self.z
        self.wavelength_index_blue = wavelength_index_blue
        self.wavelength_index_red = wavelength_index_red
        self.out_of_plane_coord = coord1
        self.inplane_coord = coord2
        
        self.set_masks1D(state,self.f,self.mf,plane,axe,coord1,coord2,wavelength_index_blue,wavelength_index_red)
        if self.trap.nblue == 0:
...
#            a.set_data(np.transpose(np.real(self.trap_2D[plane])))
    ...
    def plot_trap1D(self,plane,axis,edge1, edge2 = None, add_CP = False,Pranges=[]):
        ...
        """Addition of CP int"""
        if add_CP == True:
            C3 = self.system.get_C3(self.material,self.atomicsystem.state)
