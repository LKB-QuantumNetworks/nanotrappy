...
class ModeTable():
    """Creates dataframe that will be used in the calculations"""
    def __init__(self,E,wavelengths,powers,labels):
        return 1
...
class Outside_Simulation():
    def __init__(self,system,trap,E,wavelengths,powers,labels,x,y,z,material):
        ...
        self.z = z
        E = np.array(E)
        listEx = [E[k,:,:,0] for k in range(np.shape(E)[0])]
        listEy = [E[k,:,:,1] for k in range(np.shape(E)[0])]
        listEz = [E[k,:,:,2] for k in range(np.shape(E)[0])]
        self.dataE = pd.DataFrame({"Ex":listEx,"Ey":listEy,"Ez":listEz,"lambda" : wavelengths, "power" : powers, "label" : labels, "normalized" : [False for k in range(len(labels))]})
        # self.dataE = pd.DataFrame({"Ex":[E[0][:,:,0],E[1][:,:,0]],"Ey":[E[0][:,:,1],E[1][:,:,1]],"Ez":[E[0][:,:,2],E[1][:,:,2]],"lambda" : [780e-9,1064e-9], "power" : [1,1], "label" : ["blue","red"], "normalized" : [False,False]})
        # self.dataE = ModeTable(E,wavelengths,powers,labels)
        print(self.dataE.columns)
        self.mask_red = pd.DataFrame()
        self.mask_blue = pd.DataFrame()
        self.trap_2D = pd.DataFrame()
        
        self.material = material

        self.mask_red1D = pd.DataFrame()
        self.mask_blue1D = pd.DataFrame()
        self.trap_1D = pd.DataFrame()
        
        self.wavelength_index_blue = 0
        self.wavelength_index_red = 0
        
        self.out_of_plane_coord = 0
        self.inplane_coord = 0
        
        self.wavelengths_dict = self.trap.lambdas_trap["lambda"]
        
        # if len(self.dataE) != np.shape(self.wavelengths_dict) :
        #     raise ValueError("You must specify as many wavelengths as the number of fields in the list")

    def normalize_E(self):
            self.dataE.Ex = self.dataE.Ex/np.sqrt(self.dataE.power)
            self.dataE.Ey = self.dataE.Ey/np.sqrt(self.dataE.power)
            self.dataE.Ey = self.dataE.Ez/np.sqrt(self.dataE.power)
            self.dataE.normalized = True
            return self.dataE
    ...
    def restrictE2D(self,plane,coord_value):
        ...
        self.dataE2D.Ex = self.dataE.Ex[slc]
        self.dataE2D.Ey = self.dataE.Ey[slc]
        self.dataE2D.Ey = self.dataE.Ez[slc]
        self.dataE2D["plane"] = plane
        return dataE2D.Ex,dataE2D.Ex,dataE2D.Ex
...
#            for j in range(20):
    ...
    #####################################                   Masks in 1D                   ################################
    def getE_convert_and_compute_1D(self,state,f,mf_shift,plane,axe,out_of_plane_coord,in_plane_coord,color_index,wavelength_index):
#        E = self.E[wavelength_index]
        if len(np.shape(self.dataE.Ex[0])) == 3 :
            E = self.restrictE2D(plane,out_of_plane_coord)
        else :
            E = self.E
            
        E = self.temp_trap[(self.temp_trap["label"]==color) & (self.temp_trap["lambda"] == wavelength)].to_numpy()[0][0:2]
        if is_first(axe,plane):
            ...
            print(index)
            for i in range(len(self.coord1)):
                E0, Ep, Em = convert_fields_to_spherical_basis(E[0][i,index],E[1][i,index],E[2][i,index])
                val, vec = self.system.potential(Ep,Em,E0,state,f,self.wavelengths_dict[color_index][wavelength_index])
                res[i] = val[mf_shift]
    ...
    def set_masks1D(self,state,f,mf,plane,axe,coord1,coord2, wavelength_index_blue, wavelength_index_red):
        mf_shift = mf + f
        if  self.trap1D.loc[(self.trap1D['lambda1'] == self.lambdas_trap.loc[lambdas_trap['label']=="blue"]["lambda"].iloc[0] & self.trap1D["out of plane"] == coord1 & self.trap1D["in plane"] == coord2)] or lambdas_trap.loc[lambdas_trap['direction'] == "x"].empty:
            #no need to do that, the masks are traps in themselves. Just do 1 dataframe called trap_1D and not red and blue masks
            
            self.mask_red1D[axe,np.round(self.wavelengths_dict[1][wavelength_index_red],11),coord1,coord2]
            self.mask_blue1D[axe,np.round(self.wavelengths_dict[0][wavelength_index_blue],11),coord1,coord2]
            pass
        else :
            #here do the calculation line by line (no need for separating cases, they are already taken into account)

            res, self.vec = self.getE_convert_and_compute_1D(state,f,mf_shift,plane,axe,coord1,coord2)
    ...
    def simulate1D(self,state,f,mf,plane,axe,coord1,coord2,wavelength_index_blue,wavelength_index_red):
        """ mf has to be a list ! """
        mf = np.array(mf)
        if mf.any() > f or mf.any() < -f:
            raise ValueError("m_F should be in the interval [-F,F]")
        ...
        self.temp_trap = self.pd.merge(self.dataE, self.trap.lambdas_trap, on="lambda")
        # if lambdas_trap.groupby("label").size()["blue"] :
            
        
        self.set_masks1D(state,self.f,self.mf,plane,axe,coord1,coord2)
        
        # new_line = self.Pred * self.temp_trap[] + self.Pred * self.temp_trap[] 
        self.trap_1D = self.trap_1D.append(new_line, ignore_index=True)
    ...
    def plot_trap(self,x,y,edge,plane):
        if plane == "XY":
            ...
            a = plt.pcolormesh(x,y,np.transpose(np.real(self.trap_2D[key1])),shading = "gouraud")
            cbar = plt.colorbar(a)
        ...
        if plane == "YZ":
            fig, ax = plt.subplots()
            plt.subplots_adjust(left=0.5, bottom=0.05)
#            trap_2D = self.simulate(self.system.groundstate,self.f,self.mf,plane,wave)
            key1 = list(self.trap_2D.keys())[-1]
            a = plt.pcolormesh(x,y,np.transpose(np.real(self.trap_2D[key1])),shading = "gouraud")
#            a = plt.pcolormesh(x,y,np.transpose(np.real(trap_2D)),shading = "gouraud")
            cbar = plt.colorbar(a)
            y_edge_ind = [i for i in range(len(self.y)) if self.y[i] >= edge]
#            index_min = np.unravel_index(trap_2D[y_edge_ind[0]:y_edge_ind[-1]+1,:].argmin(),trap_2D[y_edge_ind[0]:y_edge_ind[-1]+1,:].shape)
        ...
        if self.trap.nblue != 0:
            ...
            slmbdablue = DiscreteSlider(axlmbdablue, 'lmbdablue', min(self.wavelengths_dict[0])/pu.nm, max(self.wavelengths_dict[0])/pu.nm, allowed_vals= np.array(self.wavelengths_dict[0])/pu.nm, valinit=self.wavelengths_dict[0][0],orientation='vertical')
        if self.trap.nred != 0:   
            axlmbdared = plt.axes([0.39, 0.1, 0.03, 0.75], facecolor=axcolor)
            slmbdared = DiscreteSlider(axlmbdared, 'lmbdared', min(self.wavelengths_dict[1])/pu.nm, max(self.wavelengths_dict[1])/pu.nm, allowed_vals= np.array(self.wavelengths_dict[1])/pu.nm, valinit=self.wavelengths_dict[1][0],orientation='vertical')
...
#            trap_outside = np.zeros((len(coord_outside),len(self.mf))) + CP_mf
        ...
        for k in range(len(self.mf)):
            ...
            a = a + plt.plot(coord_outside,trap_outside[:,k], color = colorVal, label = "m$_f$ = %s" %(self.mf[k]))

        if len(self.mf) == 1:
            b, = plt.plot(coord_outside,np.real(self.trap.Pblue*np.real(self.mask_blue1D[itemgetter(0,1,3,4)(key)][index_edge])), color = 'blue') #/(pu.kB*coef_norm*pu.mK))
            r, = plt.plot(coord_outside,np.real(self.trap.Pred*np.real(self.mask_red1D[itemgetter(0,2,3,4)(key)][index_edge])), color = 'red') #/(pu.kB*coef_norm*pu.mK))
...
#        ax[0].set_ylim([-2, 2])
    ...
    def plot1Dtraps_3axis(self,state,f,mf,zcoord,xcoord,edge1 = 0, edge2 = None):
        if len(self.E[0][0]['x'].shape) != 3 :
            print("3D Electric fields must be fed in the Simulation class in order to use this function")
        else : 
            ...
            slmbdablue = DiscreteSlider(axlmbdablue, 'lmbdablue', min(self.wavelengths_dict[0])/pu.nm, max(self.wavelengths_dict[0])/pu.nm, allowed_vals= np.array(self.wavelengths_dict[0])/pu.nm, valinit=self.wavelengths_dict[0][self.wavelength_index_blue],orientation='vertical')
            axlmbdared = fig.add_axes([0.2, 0.35, 0.02, 0.55], facecolor = axcolor)
            slmbdared = DiscreteSlider(axlmbdared, 'lmbdared', min(self.wavelengths_dict[1])/pu.nm, max(self.wavelengths_dict[1])/pu.nm, allowed_vals= np.array(self.wavelengths_dict[1])/pu.nm, valinit=self.wavelengths_dict[1][self.wavelength_index_red],orientation='vertical')
